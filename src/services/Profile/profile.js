...
/************************* (put) edit a profile ************************/
profilesRouter.post("/me/profileImg", JWTAuthMiddleware, cloudinaryUploader, async (req, res, next) => {
  try {
    const updatedProfile = await ProfileModel.findByIdAndUpdate(req.user._id, { image: req.file.path }, { new: true })
    if (updatedProfile) {
...
profilesRouter.post("/:profileId/picture", cloudinaryUploader, async (req, res, next) => {
  try {
    const updatedProfile = await ProfileModel.findByIdAndUpdate(req.params.profileId, { image: req.file.path }, { new: true })
    if (updatedProfile) {
      res.status(201).send(updatedProfile)
    } else {
      res.status(404).send(`Profile with id ${req.params.profileId} not found`)
...
profilesRouter.get("/me", JWTAuthMiddleware, async (req, res, next) => {
  try {
    const loggeduser = await ProfileModel.findById(req.user._id)
    if (loggeduser) {
      res.status(201).send(loggeduser)
    } else {
      res.status(404).send(`Profile with id ${req.user._id} not found`)
    }
  } catch (error) {
    next(error)
  }
...
/************************* (put) edit a profile ************************/
profilesRouter.put("/me", JWTAuthMiddleware, async (req, res, next) => {
  try {
    const updatedProfile = await ProfileModel.findByIdAndUpdate(req.user._id, req.body, { new: true })
    if (updatedProfile) {
      res.status(201).send(updatedProfile)
    } else {
      res.status(404).send(`Profile with id ${req.params.profileId} not found`)
...
//     const csvFields = ["id", "role", "company", "startDate", "endDate", "description", "area", "username", "createdAt", "updatedAt"]
...
//   try {
//     const modifiedProfile = await ProfileModel.findByIdAndUpdate(req.params.profileId, { $pull: { experiences: { _id: req.params.experienceId } } }, { new: true })
//     if (modifiedProfile) {
//       res.send()
//     } else {
//       next(createError(404, `could not find the specific profile with id ${req.params.profileId}`))
